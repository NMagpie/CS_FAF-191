...
public class WinRegistry {
    ...
    public static void createKey(long hkey, String key)
        ...
        if (hkey == HKEY_CLASSES_ROOT) {
            ...
            regCloseKey.invoke(systemRoot, new Long(ret[0]));
        } else if (hkey == HKEY_LOCAL_MACHINE) {
            ret = createKey(systemRoot, hkey, key);
            regCloseKey.invoke(systemRoot, new Long(ret[0]));
        } else if (hkey == HKEY_CURRENT_USER) {
            ret = createKey(userRoot, hkey, key);
            regCloseKey.invoke(userRoot, new Long(ret[0]));
        } else if (hkey == HKEY_USERS) {
            ret = createKey(systemRoot, hkey, key);
            regCloseKey.invoke(systemRoot, new Long(ret[0]));
        } else if (hkey == HKEY_CURRENT_CONFIG) {
    ...
    private static long deleteValue
            (Preferences root, long hkey, String key, String value)
            throws IllegalArgumentException, IllegalAccessException,
            InvocationTargetException {
        ...
        regCloseKey.invoke(root, new Long(handles[0]));
        return rc;
    }
    ...
    private static Map<String, String> readStringValues
            (Preferences root, long hkey, String key)
            throws IllegalArgumentException, IllegalAccessException,
            InvocationTargetException {
        ...
        for (int index = 0; index < count; index++) {
            byte[] name = (byte[]) regEnumValue.invoke(root, new Object[]{
                    new Long
                            (handles[0]), new Integer(index), new Long(maxlen + 1)});
    ...
    private static long[] createKey(Preferences root, long hkey, String key)
        ...
        return (long[]) regCreateKeyEx.invoke(root,
                new Object[]{new Long(hkey), toCstr(key)});
    }
    ...
    private static void writeStringValue
        ...
        regSetValueEx.invoke(root,
                new Long(handles[0]), toCstr(valueName), toCstr(value));
        regCloseKey.invoke(root, new Long(handles[0]));
    }
